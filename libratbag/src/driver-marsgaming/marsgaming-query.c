...
marsgaming_query_current_profile(struct ghostcat_device *device)
{
	...
	ghostcat_hidraw_set_feature_report(device, writedata[0], writedata, ARRAY_LENGTH(writedata));

	uint8_t readdata[16] = { 0 };
	ghostcat_hidraw_get_feature_report(device, 0x02, readdata, ARRAY_LENGTH(readdata));
...
marsgaming_query_profile_resolutions(struct ghostcat_profile *profile)
{
	...
	ghostcat_hidraw_set_feature_report(profile->device, writedata[0], writedata, ARRAY_LENGTH(writedata));

	struct marsgaming_report_resolutions report = { 0 };
	ghostcat_hidraw_get_feature_report(profile->device, 0x03, (uint8_t*)&report, sizeof(report));
...
marsgaming_query_profile_buttons(struct ghostcat_profile *profile)
{
	...
	ghostcat_hidraw_get_feature_report(profile->device, 0x04, (uint8_t*)&report, sizeof(report));
...
marsgaming_query_profile_polling_interval(struct ghostcat_profile *profile)
{
	...
	uint8_t readdata[16] = { 0 };
	ghostcat_hidraw_get_feature_report(profile->device, 0x02, readdata, ARRAY_LENGTH(readdata));
