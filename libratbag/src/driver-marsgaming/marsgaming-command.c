...
marsgaming_command_profile_set_led(struct ghostcat_led *led)
{
	...
	case GHOSTCAT_LED_CYCLE: // Not supported by the mouse, let's pretend it's breathing
		marsgaming_brightness = led->brightness * 3 / 255;
	...
	if (led->profile->is_active) {
		...
		ghostcat_hidraw_set_feature_report(led->profile->device, writedata[0], writedata, ARRAY_LENGTH(writedata));
	}
...
marsgaming_command_profile_set_resolutions(struct ghostcat_profile *profile)
{
	...
	ghostcat_hidraw_set_feature_report(profile->device, report.usb_report_id, (uint8_t*)&report, sizeof(report));
}
