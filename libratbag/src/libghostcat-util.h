...
#define LIBGHOSTCAT_EXPORT __attribute__ ((visibility("default")))
 ...
 */
ssize_t
ghostcat_utf8_to_enc(char *buf, size_t buf_len, const char *to_enc,
 ...
 */
ssize_t
ghostcat_utf8_from_enc(char *in_buf, size_t in_len, const char *from_enc,
		     char **out);
...
static inline bool
ghostcat_key_is_modifier(const unsigned int key)
{
	switch (key) {
	case KEY_LEFTALT:
	case KEY_LEFTCTRL:
