...
class ButtonsPage(Gtk.Box):
    ...
    def __init__(
        ...
        for ghostcatd_button in profile.buttons:
            button = OptionButton()
            # Set the correct label in the option button.
            self._on_button_mapping_changed(ghostcatd_button, None, button)
            button.connect("clicked", self._on_button_clicked, ghostcatd_button)
            ...
            self._mousemap.add(button, f"#button{ghostcatd_button.index}")
    ...
    def _on_button_mapping_changed(
        self,
        ghostcatd_button: RatbagdButton,
        ...
        action_type = ghostcatd_button.action_type
        if action_type == RatbagdButton.ActionType.BUTTON:
            if ghostcatd_button.mapping - 1 in RatbagdButton.BUTTON_DESCRIPTION:
                label = _(RatbagdButton.BUTTON_DESCRIPTION[ghostcatd_button.mapping - 1])
            else:
                # Translators: the {} will be replaced with the button index, e.g.
                # "Button 1 click".
                label = _("Button {} click").format(ghostcatd_button.mapping - 1)
        elif action_type == RatbagdButton.ActionType.SPECIAL:
            label = _(RatbagdButton.SPECIAL_DESCRIPTION[ghostcatd_button.special])
        elif action_type == RatbagdButton.ActionType.MACRO:
            label = _("Macro: {}").format(str(ghostcatd_button.macro))
        elif action_type == RatbagdButton.ActionType.KEY:
            label = _("Key: {}").format(evcode_to_str(ghostcatd_button.key))
    ...
    def _on_button_clicked(
        self, button: OptionButton, ghostcatd_button: RatbagdButton
    ) -> None:
        ...
        dialog = ButtonDialog(
            ...
            buttons,
            device_type,
            title=_("Configure button {}").format(ghostcatd_button.index),
            use_header_bar=True,
            transient_for=self.get_toplevel(),
        )
        dialog.connect("response", self._on_dialog_response, ghostcatd_button)
        dialog.present()
