...
from ghostcat.svg import get_svg
from .ghostcatd import RatbagdDevice
...
class MouseMap(Gtk.Container):
    ...
    def __init__(
        self,
        layer: str,
        ghostcatd_device: RatbagdDevice,
        spacing: int = 10,
        *args,
        **kwargs,
    ) -> None:
        ...
        @param layer The SVG layer whose leaders to draw, according to librsvg
                     so e.g. `#layer1`.
        @param ghostcatd_device The device that should be mapped, as
        ...
        if ghostcatd_device is None:
            raise ValueError("Device cannot be None")
        try:
            svg_bytes = get_svg(ghostcatd_device.model)
    ...
    def do_size_allocate(self, allocation: Gdk.Rectangle) -> None:
        ...
        for child in self._children:
            ...
            # Clamp leader x position to SVG bounds to prevent labels extending too far
            leader_x = min(svg_geom.x, svg_width)
            if child.is_left:
                leader_x = max(svg_geom.x, 0)
                child_allocation.x = x + leader_x - self.spacing - nat_size.width
            else:
                child_allocation.x = x + leader_x + self.spacing
            child_allocation.y = (
