...
from .ghostcatd import RatbagdResolution
...
@Gtk.Template(resource_path="/org/freedesktop/GhostCAT/ui/ResolutionRow.ui")
class ResolutionRow(Gtk.ListBoxRow):
    ...
    apply_label: Gtk.Label = Gtk.Template.Child()  # type: ignore
    disable_button: Gtk.Button = Gtk.Template.Child()  # type: ignore
    shift_button: Gtk.Button = Gtk.Template.Child()  # type: ignore
    shift_label: Gtk.Label = Gtk.Template.Child()  # type: ignore
    ...
    def _on_disable_button_toggled(self, togglebutton: Gtk.Button) -> None:
        ...
        is_disabling = togglebutton.get_active()
        self._resolution.set_disabled(is_disabling)

        # Update UI (this calls _apply_button_states which hides apply_label)
        self._on_status_changed(self._resolution, _pspec=None)

        # Show "click apply" after status update so it isn't immediately hidden
        self.apply_label.set_visible(not is_disabling)
    ...
    def _on_active_button_clicked(self, _togglebutton: Gtk.Button) -> None:
        ...
        # Manually update active labels since ghostcatd may not emit change signals
        self.resolutions_page._update_active_labels(self._resolution.index)

    @Gtk.Template.Callback("_on_shift_button_clicked")
    def _on_shift_button_clicked(self, _button: Gtk.Button) -> None:
        # The set shift button has been clicked, update RatbagdResolution.
        self._resolution.set_dpi_shift_target()
        # Manually update shift target since ghostcatd may not emit change signals
        self.resolutions_page._update_shift_target(self._resolution.index)
    ...
    def _on_status_changed(
        ...
        # DPI controls should be enabled unless the resolution is disabled
        is_enabled = not resolution.is_disabled
        self.dpi_label.set_sensitive(is_enabled)
        self.dpi_entry.set_sensitive(is_enabled)
        self.scale.set_sensitive(is_enabled)

        # Update disable button label based on current state
        if self.CAP_DISABLE:
            with self.disable_button.handler_block(self._disabled_button_handler):
                if resolution.is_disabled:
                    self.disable_button.set_active(True)
                    self.disable_button.set_label(_("Enable"))
                else:
                    self.disable_button.set_active(False)
                    self.disable_button.set_label(_("Disable"))

        # Let the ResolutionsPage handle button sensitivity
        self.resolutions_page._apply_button_states()
